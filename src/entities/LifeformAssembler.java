...
public class LifeformAssembler extends Entity {
	...
	public LifeformAssembler(int x, int y, Direction direction) {
		super(x, y, direction);
		image = new Image(ClassLoader.getSystemResourceAsStream("assets/lifeform_assembler.png"));
	}
	...
	public void update(Item[][] itemGrid) {
		...
		if (!inBounds(itemGrid, input1X, input1Y) || !inBounds(itemGrid, input2X, input2Y) || !inBounds(itemGrid, outputX, outputY))
			return;
		...
		if (in1 != null && in2 != null) {
			if (itemGrid[outputX][outputY] != null) return;
